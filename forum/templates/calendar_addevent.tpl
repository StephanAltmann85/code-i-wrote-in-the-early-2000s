...
<head>
<title>$master_board_name | {$lang->items['LANG_CALENDAR_ADDEVENT']}</title>
$headinclude
</head>
...
<table cellpadding="{$style['tableincellpadding']}" cellspacing="{$style['tableincellspacing']}" border="{$style['tableinborder']}" style="width:{$style['tableinwidth']}" class="tableinborder">
 <tr>
  <td class="tablea"><table cellpadding="0" cellspacing="0" border="0" style="width:100%">
   <tr class="tablea_fc">
    <td align="left"><span class="smallfont"><b><a href="index.php{$SID_ARG_1ST}">$master_board_name</a> &raquo; <a href="calendar.php{$SID_ARG_1ST}">{$lang->items['LANG_CALENDAR_CALENDAR']}</a> &raquo; {$lang->items['LANG_CALENDAR_ADDEVENT']}</b></span></td>
...
<table cellpadding="{$style['tableincellpadding']}" cellspacing="{$style['tableincellspacing']}" border="{$style['tableinborder']}" style="width:{$style['tableinwidth']}" class="tableinborder">
 <tr>
  <td class="tabletitle" align="left" colspan="2"><span class="normalfont"><b>{$lang->items['LANG_CALENDAR_ADDEVENT']}</b></span></td>
 </tr>
 <tr align="left">
...
<div id="newthreadOptions" class="hoverMenu">
 <ul class="smallfont">
  ...
  <if($wbbuserdata['can_use_event_smilies']==1)><then><li><input type="checkbox" id="checkbox2" name="disablesmilies" value="1" $checked[1] /><label for="checkbox2"> {$lang->items['LANG_POSTINGS_DISABLESMILIES']}</label></li></then></if>
  <if($wbbuserdata['can_use_event_html']==1)><then><li><input type="checkbox" id="checkbox3" name="disablehtml" value="1" $checked[2] /><label for="checkbox3"> {$lang->items['LANG_POSTINGS_DISABLEHTML']}</label></li></then></if>
  <if($wbbuserdata['can_use_event_bbcode']==1)><then><li><input type="checkbox" id="checkbox4" name="disablebbcode" value="1" $checked[3] /><label for="checkbox4"> {$lang->items['LANG_POSTINGS_DISABLEBBCODE']}</label></li></then></if>
  <if($wbbuserdata['can_use_event_images']==1)><then><li><input type="checkbox" id="checkbox5" name="disableimages" value="1" $checked[4] /><label for="checkbox5"> {$lang->items['LANG_POSTINGS_DISABLEIMAGES']}</label></li></then></if>
...
<p align="center">
 ...
 <input class="input" type="submit" accesskey="S" value="{$lang->items['LANG_CALENDAR_ADDEVENT']}" />
