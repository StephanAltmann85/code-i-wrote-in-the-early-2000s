...
<style type="text/css">
 ...
 #statusLayer {
	...
	height: 100%;
 	...
 	background-color: <if($style['pagebgcolor']!="")><then>{$style['pagebgcolor']};</then><else>#ffffff;</else></if>
 	color: <if($style['fontcolor']!="")><then>{$style['fontcolor']};</then><else> #000000;</else></if>
...
<if($max_attachments == -1 || $attachmentCounter < $max_attachments)><then>
 ...
 <tr><td class="mainpage" align="center">&nbsp;<table cellpadding="{$style['tableincellpadding']}" cellspacing="{$style['tableincellspacing']}" border="{$style['tableinborder']}" style="width:{$style['tableinwidth']}" class="tableinborder">
  ...
  <tr>
   <td class="tableb" align="left"><input type="hidden" name="MAX_FILE_SIZE" value="$max_attachment_size" /><input name="attachment_file" type="file" class="input" />   <input class="input" type="submit" name="submit" accesskey="S" onclick="toggle('statusLayer', 1);" value="{$lang->items['LANG_MISC_SAVE']}" /><br />
    ...
    <if($max_attachments != -1)><then>{$lang->items['LANG_MISC_ATTACHMENT_MAX_ATTACHMENTS']} <b>$max_attachments</b><br /></then></if>
    ...
    <if($LANG_MISC_ATTACHMENT_QUOTA)><then>$LANG_MISC_ATTACHMENT_QUOTA<br /></then></if>
    <if($LANG_MISC_ATTACHMENT_QUOTA_FREE)><then>$LANG_MISC_ATTACHMENT_QUOTA_FREE<br /></then></if></span></td>
  </tr>
 </table><br />
</td></tr></table></form>
</then></if>

<if($attachmentbit != '')><then>
